# AutoGolden iOS 发布中心

这里仅保存 AutoGolden 的 iPhone/iPad 发布资料：安装包、发行说明、校验值、隐私政策及支持信息；不包含应用源代码。

## 当前发布状态

- 已归档版本：0.6.2（build 9），尚未上传 TestFlight 或提交 App Store 审核。
- 支持范围：iPhone 与 iPad，最低 iOS/iPadOS 17.0。
- [下载 App Store 分发 IPA](downloads/AutoGolden-0.6.2-build9-AppStore.ipa)：供开发者上传 App Store Connect，不可通过拷贝到 iPhone 直接安装。
- 用户测试安装入口将在 TestFlight 构建处理完成后提供。
- [SHA-256 校验值](SHA256SUMS.txt) · [构建验证记录](VERIFICATION.md)

## 文档与链接

- [发行说明](RELEASE-NOTES-0.6.2.md)
- [安装与测试说明](INSTALLATION.md)
- [隐私政策](privacy.html)
- [支持页面](support.html)

AutoGolden 仅用于伦敦金现货 XAU/USD 的市场研究与教育，不连接银行、券商或支付账户，不执行交易，任何行情、点位、概率或事件分析均不构成投资建议。
